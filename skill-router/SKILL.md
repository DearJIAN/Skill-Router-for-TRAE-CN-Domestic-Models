...
```text
不要只在聊天正文里写 ROUTER_GATE。
如果 TRAE 有官方计划列表 / todo 工具，必须优先把 ROUTER_GATE 和 WORK 项写进官方计划列表。
```

### 1. Native todo first / 原生计划列表优先

When TRAE provides a native To-Do List / planning UI or a todo tool, every non-trivial task MUST create or update that native todo list before doing stage work.

For each task-specific work cluster, the native todo list should contain at least:

```text
ROUTER_GATE <checkpoint> :: <task-specific reason> :: before/after <specific work cluster>
WORK <stage/work-cluster> :: <specific user-task action>
```

Do NOT only print the task plan in chat.

A router gate counts as todo-bound only if one of these is true:

1. The gate appears in the native TRAE task plan / To-Do List.
2. The agent explicitly reports that native todo writing is unavailable and falls back to inline markdown gates.

### 2. Native todo unavailable fallback / 原生计划列表不可用时的降级

If the native todo tool or plan UI is unavailable, the agent MUST say so clearly before falling back to inline gates:

```text
NATIVE_TODO_UNAVAILABLE: true
Fallback: inline markdown router gates
Reason: <tool unavailable / platform does not expose native todo / failed tool call>
```

Do not silently fall back.

If a native todo update was not attempted and no unavailable marker is given, mark:

```text
ROUTER_VIOLATION: NATIVE_TODO_SYNC_SKIPPED
```

### 3. Native todo pre-work verification / 动手前检查原生计划列表

Before starting implementation, review, QA, docs, or final summary for a non-trivial task, verify that the native todo list contains:

- the current `ROUTER_GATE` item
- the current `WORK` item
- the next required `ROUTER_GATE` item when another stage remains

If the native todo list lacks the gate for the next work cluster, repair it before continuing:

```text
ROUTER_PLAN_REPAIR_REQUIRED: true
Reason: missing native todo gate for <stage/work-cluster>
```

Then update the native todo list and continue.
...
unless it is also mirrored into the native TRAE To-Do List or the native todo tool is explicitly unavailable.

If the agent only writes a nice-looking markdown plan but does not update native todo, mark:

```text
ROUTER_VIOLATION: MARKDOWN_PLAN_WITHOUT_NATIVE_TODO
```

### 5. Native todo status discipline / 原生计划列表状态纪律

The agent MUST keep native todo statuses aligned with router gates:

- A `WORK` item MUST NOT become in-progress before its immediately preceding `ROUTER_GATE` is completed.
- A `ROUTER_GATE` item MUST NOT be marked completed unless its stage marker was actually emitted.
- A completed work item should remain locked unless the user changes requirements, tests fail, or a regression is found.
- Mid-task router rechecks MUST resume from current progress rather than creating a fresh full plan.

If a mid-task router pass creates a brand-new unrelated todo list while previous completed items existed, mark:

```text
ROUTER_VIOLATION: NATIVE_TODO_RESET_DURING_RESUME
```

### 6. Final native todo audit / 最终原生计划列表审计

At the end of a non-trivial task, the final Stage Marker Ledger MUST include a native-todo audit column or bullet:

```text
Native todo sync: synced / unavailable fallback / violation
```

If native todo was synced, include the corresponding todo item names for each stage.

If native todo was unavailable, include the fallback reason.
...
```text
本阶段实际调用 / 使用 skills：无 — 规划阶段直接输出设计方案
```

That is allowed only for trivial or fully specified tasks. For visual/user-facing work, it is usually too weak.

### 1. Design skill minimum / 设计阶段最低参与

For UI, frontend, visual, PDF layout, dashboard, landing page, report-cover, card UI, mobile screen, or user-facing design tasks, `ROUTER_CHECKPOINT_2_PLANNING_OR_DESIGN` should normally use at least one design-oriented skill if available.

Preferred current-stage skills:

- `design-taste-frontend`
- `ui-ux-pro-max`
- `impeccable`
- `high-end-visual-design`
- `minimalist-ui` only when the requested style matches
- `brandkit` when brand/identity direction is part of the task
- `design-md` when the design system / DESIGN.md is part of the task

### 2. No empty design stage without reason / 禁止无理由空设计阶段

If no design-oriented skill is used during a UI/visual design stage, the agent MUST explain why using one of these concrete reasons:

```text
设计方向已由用户完整指定，当前阶段只做约束确认。
任务极小且非视觉重点，使用设计 skill 会造成重复。
当前 available skills 中没有合适的设计类 skill。
```

Weak reasons are invalid:
...
If a UI/visual task has a design gate but no design skill and no concrete reason, mark:

```text
ROUTER_VIOLATION: DESIGN_STAGE_SKILL_MISSING
```

### 3. Design skill is not implementation / 设计 skill 不等于提前实现

During the design stage, do not claim implementation skills as already used.

For example, in design stage:

- OK: `impeccable` for visual hierarchy / layout critique
- OK: `design-taste-frontend` for engineering-aware design direction
- NOT OK: claiming `frontend-design` was used to implement before editing files
...
That makes auditing unclear.

### 1. gstack + MCP attribution format / gstack 与 MCP 归因格式

When `gstack` is selected, loaded, or claimed for browser QA and Chrome DevTools MCP is used as the execution mechanism, the final ledger MUST attribute the QA stage as:
...
if `gstack` was selected or loaded for the QA stage.
...
if the actual browser execution was done through Chrome DevTools MCP.

### 2. Do not fake gstack browser QA / 禁止伪造 gstack 浏览器 QA

Do not claim `gstack` browser QA if neither `gstack` nor browser/MCP verification actually ran.

If `gstack` was intended but unavailable, write:

```text
gstack intended, unavailable: <reason>
QA status: PARTIAL unless another equivalent browser verification completed
```

If Chrome DevTools MCP fails and only static review is completed, write:

```text
QA status: PARTIAL
Completed: static review
Missing: browser interaction / screenshot / responsive verification
```

### 3. Final ledger consistency / 最终台账一致性

The final Stage Marker Ledger must distinguish:

- selected skills
- loaded skills
- actually used skills
- execution mechanisms / tools
...
If the row says only MCP even though `gstack` was selected or loaded, mark:

```text
ROUTER_VIOLATION: QA_ATTRIBUTION_INCONSISTENT
```
